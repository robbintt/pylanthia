    ...
    while i < len(line):
        ...
        # found some xml
            ...
            if xml_free_line_part:
                op_line.append(
                    ["text", xml_free_line_part]
                )  # modify these in place later, sometimes
...
def preprocess_tcp_lines(game_state, tcp_lines, preprocessed_lines):
    ...
    while True:
        ...
        game_state.urwid_views["urwid_tcp_view"].append(
            tcpline.decode("utf-8").replace("&gt;", ">")
        )
        preprocessed_lines.put(
            chop_xml_and_text_from_line(tcpline.strip(b"\n"))
